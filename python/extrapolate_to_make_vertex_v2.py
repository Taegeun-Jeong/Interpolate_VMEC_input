...
def main():
    if len(sys.argv) != 4:
        sys.exit("Usage: python interpolate_boundary_extrapolate.py input.eq1 input.eq2 output_file")
    file1 = sys.argv[1]
    file2 = sys.argv[2]
    ...
    # Fixed weights: weight1 = 14/12, weight2 = -2/12.
    weight1 = 14.0 / 12.0
    weight2 = -2.0 / 12.0
    ...
    # Interpolate boundary parameters (missing values are treated as 0).
    interp_boundary = {}
    for key in all_keys:
    ...
    # Use file1 as a template for the non-boundary parts.
    with open(file1, 'r') as fin:
        file1_lines = fin.readlines()
    ...
    print(f"Created {output_filename} with weight1 = {weight1:+.8E} and weight2 = {weight2:+.8E}")
